...
namespace 帮我买最大化利润
{
    class Program
    {
        ...
        public void cplex(double[][] p_rs, double[][][] d_ijk, double[][][] t_ijk, double[][] z_rs, double[] u_r, double[][][][] d_wijk, double[] pai_w, ref double cplexobj)
        {
            ...
            model.SetParam(Cplex.DoubleParam.TimeLimit, 3600);

            #region 输出
            if (model.Solve())
            {
                cplexobj = Math.Round(model.GetValue(obj), 2);
                Console.WriteLine("总利润obj = {0}   未来订单利润= {1}", Math.Round(model.GetValue(obj), 2), Math.Round(model.GetValue(obj2), 2));
                for (int r = 1; r < R + 1; r++)
                {
                    for (int s = 1; s < S + 1; s++)
                    {
                        try
                        {
                            if (model.GetValue(alpha_rs[r][s]) > 0.5)
                            {
                                Console.WriteLine("alpha_rs[r{0}][s{1}]={2}", r, s, Math.Round(model.GetValue(alpha_rs[r][s])));
                            }
                        }
                        catch
                        {
                ...
                for (int r = 1; r < R + 1; r++)
                {
                    for (int k = 0; k < K; k++)
                    {
                        try
                        {
                            if (model.GetValue(belta_rk[r][k]) > 0.5)
                            {
                                Console.WriteLine("belta_rk[r{0}][k{1}]={2}", r, k, Math.Round(model.GetValue(belta_rk[r][k])));
                            }
                        }
                        catch
                        {
                ...
                for (int r = 1; r < R + 1; r++)
                {
                    for (int s = 1; s < S + 1; s++)
                    {
                        for (int k = 0; k < K; k++)
                        {
                            try
                            {
                                if (model.GetValue(theta_rsk[r][s][k]) > 0.5)
                                {
                                    Console.WriteLine("theta_rsk[r{0}][s{1}][k{2}]={3}", r, s, k, Math.Round(model.GetValue(theta_rsk[r][s][k])));
                                }
                            }
                            catch
                            {
                ...
                for (int w = 0; w < W; w++)
                {
                    for (int o = 1; o < O + 1; o++)
                    {
                        for (int k = 0; k < K; k++)
                        {
                            try
                            {
                                if (model.GetValue(gama_wok[w][o][k]) > 0.5)
                                {
                                    Console.WriteLine("gama_wok[w{0}][o{1}][k{2}]={3}", w, o, k, Math.Round(model.GetValue(gama_wok[w][o][k])));
                                }
                            }
                            catch
                            {
                    ...
                    for (int r = 1; r < R + 1; r++)
                    {
                        for (int o = 1; o < O + 1; o++)
                        {
                            for (int k = 0; k < K; k++)
                            {
                                try
                                {
                                    if (model.GetValue(epsilon_wrok[w][r][o][k]) > 0.5)
                                    {
                                        Console.WriteLine("epsilon_wrok[w{0}][r{1}][o{2}][k{3}]={4}", w, r, o, k, Math.Round(model.GetValue(epsilon_wrok[w][r][o][k])));
                                    }
                                }
                                catch
                                {
                    ...
                    //        try
                    //        {
                    //            if (model.GetValue(tau_wok[w][o][k]) > 0.5)
                    //            {
                    //                Console.WriteLine("tau_wok[w{0}][o{1}][k{2}]={3}", w, o, k, Math.Round(model.GetValue(tau_wok[w][o][k])));
                    //            }
                    //        }
                    //        catch
                    //        {
        ...
        static void Main(string[] args)
        {
            ...
            double cplexobj = 0;
            a.cplex(p_rs, d_ijk, t_ijk, z_rs, u_r, d_wijk, pai_w, ref cplexobj);
        }
    }
}
